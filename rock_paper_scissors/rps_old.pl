...
turn:-
	thread_create(choose, ID_CHOOSE, [alias(choice), inherit_from(main)]),
	%thread_create(countdown(3), ID_COUNTDOWN, [at_exit(cancel_choose)]),
	catch(thread_join(ID_CHOOSE), _ , true),
	%thread_join(ID_COUNTDOWN),
	ai_pick(AI_ITEM),
	item(PLAYER_ITEM),!,
